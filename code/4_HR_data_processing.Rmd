...
See *Estimating lobster resting heart rates* in the methods section of the manuscript for additional details.
...
# pdf(paste("figures/heart_rate/indiv_hr", "time_plot", Sys.Date(),".pdf", sep = "_"), onefile = TRUE, width = 9, height = 6)
...
for(i in 1:length(lob_ids)) {
  ...
  pdf(paste("figures/heart_rate", lob_id, "ecg", Sys.Date(),".pdf"), onefile = TRUE, width = 9, height = 6)
  make_ecg_pdf(lob_id) # do make_ecg_pdf()
  dev.off()
}
```
...
####**Part 5.** Select only usable lobsters; remove anomolous measurements (noted in `poor_signal_inventory.csv`); replace clear instances of arrested heart rates with "0" (noted in `hr_stop_inventory.csv`)
```{r}
##############################
...
for (i in 1:nrow(all_bpm)) {
  ...
  if (nrow(poor_signal_subset) > 0) {
    # clean_count should be equal to the number of rows in cleaning_inventory
    clean_count <- clean_count + 1
    all_bpm[i,][1] <- as.character("NA")
    all_bpm[i,][2] <- as.character("NA")
    all_bpm[i,][3] <- as.character("NA")
    all_bpm[i,][4] <- as.character("NA")
    all_bpm[i,][5] <- as.factor("NA")
    all_bpm[i,][6] <- as.character("NA")
...
# print any rows that are missing from cleaning_inventory
...
# print old and new data to make sure it's replacing hr with correct value from inventory
# for (i in 1:nrow(hr_inventory)) {
#   current_row <- hr_inventory[i,]
#   ts <- as.character(current_row[2][[1]])
#   lob_id <- as.character(current_row[1][[1]])
#   data_subset <- all_bpm %>% 
#     filter(lobster_id == lob_id) %>% 
