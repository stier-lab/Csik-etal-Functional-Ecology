...
#maximum smr
smrmax<-max(smr11_avg,smr16_avg,smr21_avg,smr26_avg)
pofmax<-data.frame("temperature"=c(11,16,21,26),"smrpofmax"=round(100*c(as.numeric(c(smr11_avg,smr16_avg,smr21_avg,smr26_avg))/smrmax)))
...
aasmax<-max(aas11_avg,aas16_avg,aas21_avg,aas26_avg)
pofmax$aaspofmax<-round(100*c(as.numeric(c(aas11_avg,aas16_avg,aas21_avg,aas26_avg))/aasmax))
...
mcmax<-max(mc11_avg,mc16_avg,mc21_avg,mc26_avg)
pofmax$mcpofmax<-round(100*c(as.numeric(c(mc11_avg,mc16_avg,mc21_avg,mc26_avg))/mcmax))

pofmax2<-
pofmax%>%
pivot_longer(!temperature,names_to = "measurement", values_to = "count")

ggplot(data=pofmax2,aes(x=temperature,y=count,
                        group=measurement,colour=measurement
                        ))+
  geom_point()+
  geom_smooth(method="lm",se=FALSE)+
  theme_classic()+
  scale_colour_discrete(name = "Measurement", labels = c("AAS", "Consumption", "SMR"))+
  ylab("% of maximum")+
  xlab("Temperature")
...
((smr26_avg - smr11_avg)/smr11_avg * 100)
