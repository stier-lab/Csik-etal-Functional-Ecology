...
mr_no_mass <- metabolic_rates %>% 
  mutate(SMR = BW * SMR_low15quant,
         MMR = BW * mmr_overall,
         AS = BW * AS_SMR_low15quant) %>% 
  select(ID, temp, BW, SMR, MMR, AS) %>% 
  gather(SMR, MMR, AS, key = trait, value = mr_no_mass) %>% 
  mutate(trait = fct_relevel(trait, "SMR", "MMR", "AS"),
...
SMR_summary <- metabolic_rates %>%
  ...
  dplyr::group_by(temp) %>% 
  dplyr::summarise(
    SMR_avg = round(mean(SMR_low15quant, na.rm = T),3),
    SMR_sd = round(sd(SMR_low15quant, na.rm = T),3),
    SMR_se = round(SMR_sd/sqrt(length(SMR_low15quant)),3),
    SMR_cv = SMR_sd/SMR_avg,
    SMR_min = round(min(SMR_low15quant),3),
    SMR_max = round(max(SMR_low15quant),3),
    error = qnorm(0.975) * sd(SMR_low15quant)/sqrt(length(SMR_low15quant)),
...
MMR_summary <- metabolic_rates %>%
  ...
  dplyr::group_by(temp) %>% 
  dplyr::summarize(
    MMR_avg = round(mean(mmr_overall, na.rm = T), 3),
    MMR_sd = round(sd(mmr_overall, na.rm = T),3),
    MMR_se = round(MMR_sd/sqrt(length(mmr_overall)),3),
    MMR_cv =  MMR_sd/MMR_avg,
    MMR_min = round(min(mmr_overall),3),
    MMR_max = round(max(mmr_overall),3),
    error = qnorm(0.975) * sd(mmr_overall)/sqrt(length(mmr_overall)),
...
AAS_summary <- metabolic_rates %>%
  ...
  dplyr::group_by(temp) %>% 
  dplyr::summarize(
    AS_avg = round(mean(AS_SMR_low15quant, na.rm = T),3),
    AS_sd = round(sd(AS_SMR_low15quant, na.rm = T),3),
    AS_se = round(AS_sd/sqrt(length(AS_SMR_low15quant)),3),
    AS_cv = AS_sd/AS_avg,
    AS_min = round(min(AS_SMR_low15quant),3),
    AS_max = round(max(AS_SMR_low15quant),3),
    error = qnorm(0.975) * sd(AS_SMR_low15quant)/sqrt(length(AS_SMR_low15quant)), 
...
FAS_summary <- metabolic_rates %>%
  ...
  dplyr::group_by(temp) %>% 
  dplyr::summarize(
    FAS_avg = round(mean(FAS_q15, na.rm = T), 3),
    FAS_sd = round(sd(FAS_q15, na.rm = T), 3),
    FAS_se = FAS_sd/sqrt(length(FAS_q15)),
    FAS_cv = FAS_sd/FAS_avg,
    FAS_min = min(FAS_q15),
    FAS_max = max(FAS_q15),
    error = qnorm(0.975) * sd(FAS_q15)/sqrt(length(FAS_q15)),
