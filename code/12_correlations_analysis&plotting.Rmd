...
shapiro.test(metabolism_foraging_data$SMR_low15quant) # p = 0.1281
shapiro.test(metabolism_foraging_data$mmr_overall) # p = 0.8653
shapiro.test(metabolism_foraging_data$AS_SMR_low15quant) # p = 0.9025
shapiro.test(metabolism_foraging_data$FAS_q15) # p = 0.001497* bc of 11C
...
ggqqplot(metabolism_foraging_data$SMR_low15quant, ylab = "SMR") # pretty good
ggqqplot(metabolism_foraging_data$mmr_overall, ylab = "MMR") # great
ggqqplot(metabolism_foraging_data$AS_SMR_low15quant, ylab = "AAS") # great
ggqqplot(metabolism_foraging_data$FAS_q15, ylab = "FAS") # great except for 11C
...
smr_cor <- cor.test(metabolism_foraging_data$SMR_low15quant, metabolism_foraging_data$avg_eaten, method = c("pearson"))
...
mmr_cor <- cor.test(metabolism_foraging_data$mmr_overall, metabolism_foraging_data$avg_eaten, method = c("pearson"))
...
aas_cor <- cor.test(metabolism_foraging_data$AS_SMR_low15quant, metabolism_foraging_data$avg_eaten, method = c("pearson"))
...
fas_cor <- cor.test(metabolism_foraging_data$FAS_q15, metabolism_foraging_data$avg_eaten, method = c("pearson"))
...
# write.csv(correlation_table, here::here("data", "correlations", "outputs", "correlations.csv"))
```
...
NEW_metabolism_foraging_data <- metabolism_foraging_data %>% 
  select(SMR_low15quant, mmr_overall, AS_SMR_low15quant, FAS_q15, avg_eaten) %>% 
  rename(SMR = SMR_low15quant, MMR = mmr_overall, AAS = AS_SMR_low15quant, FAS = FAS_q15, `Average Consumed` = avg_eaten)
...
smr_corr_plot <- ggplot(metabolism_foraging_data, aes(x = SMR_low15quant, y = avg_eaten)) + 
...
mmr_corr_plot <- ggplot(metabolism_foraging_data, aes(x = mmr_overall, y = avg_eaten)) + 
...
aas_corr_plot <- ggplot(metabolism_foraging_data, aes(x = AS_SMR_low15quant, y = avg_eaten)) + 
  geom_errorbar(aes(ymin = avg_eaten - sd_eaten, ymax = avg_eaten + sd_eaten), width = 0.07, color = "lightgray") +
  geom_point(aes(color = temp, shape = temp, size = temp)) + 
  geom_smooth(method = "lm", color = "gray36", size = 0.75, level = 0.95) +
  scale_color_manual(values = c("lightslategray", "lightblue", "lightcoral", "indianred4"), name = "Temperature (°C)") +
...
fas_corr_plot <- ggplot(metabolism_foraging_data, aes(x = mmr_overall/SMR_low15quant, y = avg_eaten)) + 
  ...
  annotate(geom = "curve", x = 3.8, y = 4.45, xend = 3.2, yend = 14.0,
           curvature = 0.2, arrow = arrow(length = unit(3, "mm"))) +
  annotate(geom = "text", x = 3.75, y = 4.4, label = "IV19", hjust = "right")
...
correlations_plot <- cowplot::plot_grid(smr_marg, mmr_marg, aas_marg, fas_marg, nrow = 2, labels = "AUTO", axis = "tblr", rel_widths = c(0.85, 0.85, 0.85, 0.85))
